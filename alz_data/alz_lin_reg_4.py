...
mappings = {
    ...
    'Genetic Risk Factor (APOE-ε4 allele)': {'Yes': 1, 'No': 0},
...
# Plot correlation between each variable and the target variable
for i, column in enumerate(df.columns):
    if column != target_variable:
        plt.subplot(5, 5, i + 1)
        sns.regplot(x=df[column], y=df[target_variable], scatter_kws={'s':10})
        plt.xlabel(column)
